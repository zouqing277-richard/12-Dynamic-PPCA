...
class VARResidual:
    ...
    def from_true_model(cls, ic_model: dict) -> "VARResidual":
        """
        Build a VARResidual monitor from the TRUE IC model parameters.

        The observed process x_t generated by DyPPCA has:
          Cov(x_t)       = Sigma_x  = A0 A0^T + sigma0 I
          Cov(x_t,x_{t-1}) = C1     = A0 B0 A0^T

        The true VAR(1) coefficient is:
          Phi = C1 * Sigma_x^{-1}

        and the true innovation covariance is:
        ...
        Sigma_x = A0 @ A0.T + sigma0 * np.eye(p)   # true marginal covariance
        C1      = A0 @ B0 @ A0.T                    # true lag-1 cross-cov
        Phi     = C1 @ np.linalg.inv(Sigma_x)       # true VAR coefficient
        c       = nu0 - Phi @ nu0                   # true intercept (= 0 when nu0=0)
        Sigma_e = Sigma_x - Phi @ Sigma_x @ Phi.T   # innovation covariance
        Sigma_e = (Sigma_e + Sigma_e.T) / 2         # symmetrize
        Sigma_e += 1e-8 * np.eye(p)                 # numerical safety
